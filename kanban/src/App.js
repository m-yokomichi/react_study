...
function App() {
  ...
  return (
    <>
      { shownCreateCardListModal ? <CreateModal onAdd={(newCardList) => updateState(addCardList(newCardList))} onClose={() => updateState(closeCreateCardListModal())}/> : <></> }
      { shownCreateCardModal ? <CreateModal onAdd={(newCard) => updateState(addCard(newCard))} onClose={() => updateState(closeCreateCardModal())}/> : <></> }
      <Navigation />
      <SCardLists>
        <ReactSortable
          list={cardLists}
